...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # gradient of W2
        softmax[np.arange(N), y] -= 1 # (softmax)'
        softmax /= N

        dW2 = layer1.T.dot(softmax)

        # gradient of b2
        db2 = np.sum(softmax, axis = 0)

        # gradient of W1
        dReLU = np.zeros_like(layer1)
        dReLU[layer1 > 0] = 1

        sWR = softmax.dot(W2.T) * dReLU
        dW1 = X.T.dot(sWR)

        # gradient of b1
        db1 = np.sum(sWR, axis=0)
